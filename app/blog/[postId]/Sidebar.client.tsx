...
const Sidebar = () => {
  ...
  let clickHandler = (e: Event, observer: IntersectionObserver) => {
    ...
    if (!href || !href.startsWith("#") || href.length <= 1) {
      setContents(
        Array.from(document.querySelectorAll("h1, h2, h3, h4, h5")).map(
          (el) => ({ id: el.id, content: el.textContent || "" }),
        ),
      );
      return;
    }
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        for (let i = 0; i < entries.length; i++) {
          ...
          const ddElement = document.querySelector(
            `.ddClass a[href="#${entry.target.id}"]`,
          ) as HTMLElement;
          ...
          if (
            ...
            !ddElement.parentElement?.classList.contains("green")
          ) {
