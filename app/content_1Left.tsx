...
const Content_1Left = () => {
  ...
  function handleClickUp(event: any) {
    ...
    up >= 0
      ? setUp((pre: number) => pre)
      : setUp((pre: number) => pre + scrollVolume + 6);
  ...
  function mouseOver() {
    ...
    daysEl.forEach((element) => {
      element.addEventListener("mouseover", () => {
        let daysEl = document.querySelectorAll(".days");
        let rightContentwheelFlag1El = document.querySelector(
  ...
  const filteredData = timelineData.filter(
    (data): data is (typeof timelineData)[0] => data !== undefined,
  );
