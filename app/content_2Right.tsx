				...
				<div className='pagination'>
					<button
						onClick={() => {
							...
							localStorageOrigin('setItem', { PageNationNumber: currentPage - 1 })
							...
							if (currentPage <= 1 && Content.length % ViewNumber !== 0) {
								...
								localStorageOrigin('setItem', { PageNationNumber: Math.ceil(Content.length / ViewNumber) })
								const newContent = [
								...
								const newContent = [...Content.slice(Content.length - ViewNumber, Content.length), ...Content.slice(0, Content.length - ViewNumber)]
								const newImages = [...Images.slice(Images.length - ViewNumber, Images.length), ...Images.slice(0, Images.length - ViewNumber)]
								const newCategorys = [...Categorys.slice(Categorys.length - ViewNumber, Categorys.length), ...Categorys.slice(0, Categorys.length - ViewNumber)]
								const newIds = [...Ids.slice(Ids.length - ViewNumber, Ids.length), ...Ids.slice(0, Ids.length - ViewNumber)]
								const newDiscribe = [...discribe.slice(discribe.length - ViewNumber, discribe.length), ...discribe.slice(0, discribe.length - ViewNumber)]
								const newTitle = [...title.slice(title.length - ViewNumber, title.length), ...title.slice(0, title.length - ViewNumber)]
						...
						{Array.from({ length: Math.ceil(Content.length / ViewNumber) }, (_, i) => i + 1).map((pageNum) => (
							<button
								...
								onClick={() => {
									...
									localStorageOrigin('setItem', { PageNationNumber: pageNum })
									...
									const newContent = [...originalContentDupe.slice(start, end), ...originalContentDupe.slice(end), ...originalContentDupe.slice(0, start)]
									const newImages = [...originalImagesDupe.slice(start, end), ...originalImagesDupe.slice(end), ...originalImagesDupe.slice(0, start)]
									const newCategorys = [...originalCategorysDupe.slice(start, end), ...originalCategorysDupe.slice(end), ...originalCategorysDupe.slice(0, start)]
									const newIds = [...originalIdsDupe.slice(start, end), ...originalIdsDupe.slice(end), ...originalIdsDupe.slice(0, start)]
									const newDiscribe = [...originalDiscribeDupe.slice(start, end), ...originalDiscribeDupe.slice(end), ...originalDiscribeDupe.slice(0, start)]
									const newTitle = [...originalTitleDupe.slice(start, end), ...originalTitleDupe.slice(end), ...originalTitleDupe.slice(0, start)]
					...
					<button
						onClick={() => {
							...
							if (currentPage >= Math.ceil(Content.length / ViewNumber)) {
								setCurrentPage(1)
								localStorageOrigin('setItem', { PageNationNumber: 1 })
									...
									<div className='twoCard' key={index}>
										...
										<Link href={`/blog/${Ids[index]}`}>
											<div className='imageEl'>
												<Image src={Images[index] !== undefined ? Images[index].url : '/logoPacks/typescript.svg'} alt='Image' layout='fill' objectFit='cover' objectPosition='top right' />
											</div>
										</Link>
										<Link href={`/blog/${Ids[index]}`}>
