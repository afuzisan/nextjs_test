...
const SwipeComponent = () => {
  ...
  function logSwipeEnd(event: TouchEvent) {
    ...
    if (end !== null && start !== null) {
      const scrollData = JSON.parse(
        localStorage.getItem("scrollData") || '{"wheelFlag":1,"deltaTotal":0}',
      );
      ...
      if (distanceY > minimumDistance || distanceX > minimumDistance) {
        // 最小幅以上動かした時のみ判定
        if (0 > end.y - start.y && distanceX < minimumDistance) {
          ...
          let [newDeltaTotal, newWheelFlag] = leftSwipe(
            wheelFlag,
            deltaTotal,
            "left",
            timeLine,
          ...
          let [newDeltaTotal, newWheelFlag] = rightSwipe(
            wheelFlag,
            deltaTotal,
            "right",
            timeLine,
  ...
  function leftSwipe(
    ...
    if (wheelFlag == 1) {
      ...
      if (timeLineResult) {
        ...
        let rightContentwheelFlag2 = document.querySelector(
          ".rightContentwheelFlag2",
        ) as HTMLElement;
        let leftContentwheelFlag2 = document.querySelector(
          ".leftContentwheelFlag2",
        ) as HTMLElement;
      ...
      try {
        ...
        if (result1 !== null && result1 !== undefined && result1.length > 1) {
          result1.forEach((element, i) => {
            ...
            let elementAsCollection =
              elementAsUnknown as HTMLCollectionOf<Element>;
  ...
  function rightANDleftContentCreate(
    flagEl: string,
